...
public class PitchingProcessor implements Serializable {
	...
	public void startProcess(DbReader reader) throws IOException {
		...
		regular = regular.join(post, "playerID")
				.groupBy("playerID", "YearID", "RegularavgERA", "PostavgERA", "RegularWinPercent", "PostWinPercent",
						"RegularLossPercent", "PostLossPercent")
				// .agg(functions.avg("PostavgERA"),
				// functions.sum("RegularWins"),functions.sum("PostWins"),functions.sum("RegularLosses"));
				.count();
		regular.show();
		Dataset<Pitching> pitching = regular.map(new MapFunction<Row, Pitching>() {
	...
	private Dataset<Row> getRegularSeasonPitching(DbReader reader, int num) {
		...
		ret = ret.withColumnRenamed("totalWins", "RegularWins").withColumnRenamed("avgERA", "RegularavgERA")
				.withColumnRenamed("YearID", "RegularYearID").withColumnRenamed("totalLoss", "RegularLosses")
	...
	private Dataset<Row> getPostSeasonPitching(DbReader reader, int num) {
		Dataset<Row> ret = getData(reader, num, "pitchingpost");// .
		ret = ret.withColumnRenamed("totalWins", "PostWins").withColumnRenamed("avgERA", "PostavgERA")
				.withColumnRenamed("totalLoss", "PostLosses").withColumnRenamed("WinPercent", "PostWinPercent")
	...
	private Dataset<Row> getData(DbReader reader, int num, String table) {
		...
		Dataset<Row> eraGroup = regular.withColumn("ERAC", regular.col("ERA").cast(DataTypes.DoubleType))
				.filter(functions.expr("ERAC > 0")).groupBy("ERAC", "playerID").avg("ERAC")
				// .withColumn("samp", functions.avg("ERA"))//.
				.withColumnRenamed("avg(ERAC)", "avgERA").sort(new Column("avgERA")).limit(num);

		regular = regular.join(eraGroup, "playerID").groupBy("YearID", "playerID", "avgERA")
				.agg(functions.sum("W"), functions.sum("L")).withColumnRenamed("sum(W)", "totalWins")
				.withColumnRenamed("sum(L)", "totalLoss").withColumn("total", functions.expr("totalWins+totalLoss"))
